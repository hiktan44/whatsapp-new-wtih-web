...
/**
 * WhatsApp Web oturumunu kapat (API route'lar ile uyumluluk için)
 * TR: logoutWaWeb adı eski route'larla uyumluluk sağlar.
 * EN: Kept for backward compatibility with existing routes.
 */
...
export async function getContacts(): Promise<any[]> {
  ...
  try {
    ...
    // isMyContact kontrolü sorunlu, sadece geçerli kişileri filtrele
    return contacts
      .filter((c: any) => {
        // Grup değil ve telefon numarası var
        return !c.isGroup && c.id && c.id.user && c.id.user.length > 5;
      })
      .map((c: any) => ({
        id: c.id._serialized,
